...
FROM nvidia/cuda:12.1.0-devel-ubuntu20.04 as nvidia_base
LABEL maintainer="Ryu Taehun, <xogns2079@gmail.com>"
LABEL version="1.0"
LABEL description="ROS2 Foxy with YOLOv8 segmentation"

RUN echo 'export PATH=$PATH:/usr/local/cuda-12.1/bin' >> /etc/bash.bashrc
RUN echo 'export CUDADIR=/usr/local/cuda-12.1' >> /etc/bash.bashrc
RUN echo 'export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/usr/local/cuda-12.1/lib64/' >> /etc/bash.bashrc
