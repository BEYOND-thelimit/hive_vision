...
def generate_launch_description():
    ...
    model_cmd = DeclareLaunchArgument(
        "model",
        default_value="yolov8m.pt",  # 모델 경로
    ...
    mytracking_node_cmd = Node(
        package="yolov8_ros",  # 패키지 이름 확인
        executable="mytracking_node",  # 실행 파일 이름
        name="mytracking_node",
        ...
        remappings=[
            ("camera/color/image_raw", input_image_topic)  # 필요 시 리매핑 추가
        ]
    )
